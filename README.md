...
8. Provisioning WhatsApp Evolution (istanza condivisa di piattaforma), QR, webhook autenticato e risposta automatica usando profilo + knowledge base — instradata in base ai **numeri WhatsApp personali** che ogni account registra (piano base: 1 incluso; piano pro: 2 inclusi; extra a 5€/mese ciascuno). Un numero registrato può essere quello di un'altra persona autorizzata a scrivere al bot, oppure lo stesso numero connesso via QR: in quel caso la chat WhatsApp "Messaggi a te stesso" su quel telefono diventa una conversazione con l'assistente (il webhook riconosce questo caso solo se il numero coincide con `WHATSAPP_BOT_NUMBER`/`WHATSAPP_PUBLIC_NUMBER`; ogni altro messaggio scritto manualmente dal titolare in chat con terzi resta ignorato, per non intromettersi in conversazioni personali). Un identificatore di messaggio inviato dal bot viene tenuto a mente per pochi minuti per riconoscere ed escludere l'eco della propria stessa risposta in quella chat, evitando cicli infiniti.
